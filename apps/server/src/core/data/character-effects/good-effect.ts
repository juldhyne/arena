...
export class GoodEffect extends CharacterEffect {
  constructor(sourceId: string, turnsLeft: number = 3) {
    super("good", sourceId, turnsLeft);
  }
  ...
  onAfterUpdateApplied(
    ...
    if (this.turnsLeft == 1) {
      return [new HealUpdate(this.sourceId, target.id, 3)];
    }
  ...
  decrementTurnsLeft(): CharacterEffect {
    return new GoodEffect(this.sourceId, this.turnsLeft - 1);
  }
}

...
import { ResurrectionEffect } from "../character-effects/resurrection-effect";
import { AddEffectUpdate } from "../character-updates/add-effect-update";
import { PointAndClick } from "./point-and-click";

export class ResurrectionSkillExecutor extends SkillExecutor {
  execute(
    source: Character,
    state: GameState,
    ...
    if (target) {
      const effect = new ResurrectionEffect(source.id);
